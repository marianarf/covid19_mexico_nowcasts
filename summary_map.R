...
data <- readRDS('data/summary_data-2020-05-21.rds') %>%
  ...
  filter(!grepl('Effective reproduction no.', metric)) %>%
...
map_data <- stringdist_left_join(ids, data, by=c(NOM_ENT='region'), max_dist=1, ignore_case=TRUE) %>%
  as.data.frame()
...
# Write data
geojson_write(nowcasts_map, file='output_maps/nowcasts-2020-05-21')
